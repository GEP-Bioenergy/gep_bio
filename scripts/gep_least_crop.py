...
# Get top crop  
def get_crop(state, crop):
 
    return crop.loc[crop['States'] == state, 'Crop'].iloc[0]
...
def get_percentage_area(state, crop):
    
    return crop.loc[crop['States'] == state, 'PercentageArea'].iloc[0]
...
'''
def crop_land_calc(sub_data, top_crop, least_crop, crop_data):
    
    # Add top crop_data to sub_data
    sub_data['TopCrop'] = [ get_crop(state, top_crop) for state in sub_data['Admin1']]
    sub_data['PercentageArea'] = [ get_percentage_area(state, top_crop) for state in sub_data['Admin1']]
    
    # Add least crop_data to sub_data
    sub_data['LeastCrop'] = [ get_crop(state, least_crop) for state in sub_data['Admin1']]
    sub_data['L_PercentageArea'] = [ get_percentage_area(state, least_crop) for state in sub_data['Admin1']]
    ...
    #least
    sub_data['L_LHV'] = [ get_LHV_crop(crop, crop_data) for crop in sub_data['LeastCrop']]
    sub_data['L_RPR'] = [ get_RPR_crop(crop, crop_data) for crop in sub_data['LeastCrop']]
    sub_data['L_CropYield'] = [ get_yield_crop(crop, crop_data) for crop in sub_data['LeastCrop']]
    ...
    # Calculate residue weight based on least crop LHV
    sub_data['L_WeightOfRes']= (sub_data['TotalEnergyPerCell']* 3.6)/(0.3 * sub_data['L_LHV'] * 1000) #Ton 
    
    # Determine the weight of needed top crop product (tons) @40% Availability
    sub_data['WeightOfProd40']=sub_data['WeightOfRes']/(RESIDUE_AVAILABILITY_FACTOR* sub_data['RPR'])
     # Determine the weight of needed least crop product (tons) @40% Availability
    sub_data['L_WeightOfProd40']=sub_data['L_WeightOfRes']/(RESIDUE_AVAILABILITY_FACTOR* sub_data['L_RPR'])
    
    # Estimate top crop cropping area using yield
    sub_data['CropLandArea']=sub_data['WeightOfProd40']/(sub_data['CropYield']) #ha
    # Estimate least crop cropping area using yield
    sub_data['L_CropLandArea']=sub_data['L_WeightOfProd40']/(sub_data['L_CropYield']) #ha
    ...
    # Estimate cropping area based on cultivation percentage of LeastCrop
    sub_data['L_CultivationArea']=sub_data['L_CropLandArea']/(sub_data['L_PercentageArea']) #ha
...
def npv_fuel_cost(sub_data):
    ...
    # Determining the delivered Fuel cost per Tonne ($/ton) for top crop
    sub_data['Ft_PerTonRes'] = sub_data['Ft']/sub_data['WeightOfRes']
    # Determining the delivered Fuel cost per Tonne ($/ton) for least crop
    sub_data['L_Ft_PerTonRes'] = sub_data['Ft']/sub_data['L_WeightOfRes']
...
def data_postprocessing(sub_data, scenario, fuelCDF):
    ...
    fuelCDF.to_csv('CDF_test.csv', index=False)
